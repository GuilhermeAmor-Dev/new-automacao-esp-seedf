...
export default function EspEditor() {
  ...
  const isCadernoMode = location.startsWith("/caderno");
  const routePrefix = isCadernoMode ? "/caderno" : "/esp";
  const ownerId = espId;
  ...
  const [legislacoesSelecionadas, setLegislacoesSelecionadas] = useState<string[]>([""]);
  ...
  useEffect(() => {
    ...
    if (!isCadernoMode && esp) {
      ...
      setLegislacoesSelecionadas((esp.legislacao || "").split("\n").filter(Boolean).length ? (esp.legislacao || "").split("\n").filter(Boolean) : [""]);
      setReferenciasSelecionadas((esp.referencias || "").split("\n").filter(Boolean).length ? (esp.referencias || "").split("\n").filter(Boolean) : [""]);
      setNormasSelecionadas(((esp as any).normas || "").split("\n").filter(Boolean).length ? ((esp as any).normas || "").split("\n").filter(Boolean) : [""]);
    }
  ...
  const handleSave = () => {
    ...
    if (isCadernoMode) {
      ...
      form.handleSubmit((data) => {
        // Sync textual fields with selected lists
        form.setValue("legislacao", legislacoesSelecionadas.filter(Boolean).join("\n"));
        form.setValue("referencias", referenciasSelecionadas.filter(Boolean).join("\n"));
        form.setValue("normas", normasSelecionadas.filter(Boolean).join("\n"));
  ...
  const { data: filesData } = useQuery<{ files: any[] }>({
    queryKey: ["/api/files", ownerId, "files"],
    queryFn: async () => {
      const token = localStorage.getItem("esp_auth_token");
      const response = await fetch(`/api/files/${ownerId}/files`, {
        credentials: "include",
        headers: token ? { "Authorization": `Bearer ${token}` } : {},
      });
  ...
  const handleFilesSelected = async (files: File[]) => {
    ...
    try {
      ...
      if (espId) {
        if (isCadernoMode) {
          formData.append("cadernoId", espId);
        } else {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/files", ownerId, "files"] });
            ...
            {tabs.map((tab) => (
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const targetId = espId || 'novo';
                  setLocation(`${routePrefix}/${targetId}/${tab.id}`);
                      ...
                      return (
                        <div key={`leg-${idx}`} className="flex items-center gap-2">
                          <Select
                            value={val}
                            onValueChange={(v) => {
                              const next = [...legislacoesSelecionadas];
                              next[idx] = v;
                              setLegislacoesSelecionadas(next);
                            }}
                          >
                            <SelectTrigger className="w-full">
                              <SelectValue placeholder="Selecione legislação" />
                            </SelectTrigger>
                            <SelectContent>
                          ...
                          {idx > 0 && (
                            <Button
                              ...
                              size="icon"
                              onClick={() =>
                                setLegislacoesSelecionadas(legislacoesSelecionadas.filter((_, i) => i !== idx))
                              }
                            >
                              <Trash2 className="h-4 w-4" />
                            </Button>
                      ...
                      return (
                        <div key={`ref-${idx}`} className="flex items-center gap-2">
                          <Select
                            value={val}
                            onValueChange={(v) => {
                              const next = [...referenciasSelecionadas];
                              next[idx] = v;
                              setReferenciasSelecionadas(next);
                            }}
                          >
                            <SelectTrigger className="w-full">
                          ...
                          {idx > 0 && (
                            <Button
                              ...
                              size="icon"
                              onClick={() =>
                                setReferenciasSelecionadas(referenciasSelecionadas.filter((_, i) => i !== idx))
                              }
                            >
                              <Trash2 className="h-4 w-4" />
                            </Button>
                      ...
                      return (
                        <div key={`norma-${idx}`} className="flex items-center gap-2">
                          <Select
                            value={val}
                            onValueChange={(v) => {
                              const next = [...normasSelecionadas];
                              next[idx] = v;
                              setNormasSelecionadas(next);
                            }}
                          >
                            <SelectTrigger className="w-full">
                              <SelectValue placeholder="Selecione norma" />
                            </SelectTrigger>
                            <SelectContent>
                          ...
                          {idx > 0 && (
                            <Button
                              ...
                              size="icon"
                              onClick={() =>
                                setNormasSelecionadas(normasSelecionadas.filter((_, i) => i !== idx))
                              }
                            >
                              <Trash2 className="h-4 w-4" />
                            </Button>
